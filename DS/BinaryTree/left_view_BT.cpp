...
//function to show left view of BT
template <typename T>
void leftView(Node<T> *temp,int curr_level){
    //to check if curr level is taken or not
    static int level=0;

    if(temp==NULL)
        return;

    //if curr level is being visited first time then we got an left node
    if(level<curr_level){
        //change level to curr level so we can avoid other nodes of this level
        level=curr_level;
        cout<<temp->key<<endl;
    }
    leftView(temp->left,curr_level+1);//left subtree
    leftView(temp->right,curr_level+1);//right subtree
}
