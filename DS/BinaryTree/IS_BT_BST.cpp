...
//function to check if BT is BST or not
template <typename T>
bool isBST(Node<T> *temp,int min_node,int max_node){
    if(temp==NULL)
        return true;

    //if node value is between range then go ahead
    if(temp->key<=max_node and temp->key>=min_node){
        bool ans=isBST(temp->left,min_node,temp->key);
        if(ans)
            ans=isBST(temp->right,temp->key,max_node);
        return ans;
    }
    //if node value is not in range then not BST return false
    else{
        cout<<"\nBST killer : "<<temp->key<<" "<<min_node<<" "<<max_node<<endl;
        return false;
    }   
}
int main(int argc, char const *argv[])
{
    ...
    inorder(head);
    bool ans=isBST(head,0,100);
    
    cout<<(ans ? "Tree is BST..." : "Tree is not BST...");
    return 0;
}
